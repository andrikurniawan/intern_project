...
class TCPDF {
	 ...
	 *	 <li>CNZ: Clipping mode (using the even-odd rule to deter which regions lie inside the clipping path).</li>
	 *	 <li>CEO: Clipping mode (using the nonzero winding number rule to deter which regions lie inside the clipping path).</li>
	 * </ul>
	...
	protected function utf8Bidi($ta, $str='', $forcertl=false) {
		...
		for ($i=0; $i < $numchars; ++$i) {
			if ($ta[$i] == $this->unicode->uni_RLE) {
				...
				// X7. With each PDF, deter the matching embedding or override code. If there was a valid matching code, restore (pop) the last remembered (pushed) embedding level and directional override.
				if (count($remember)) {
		...
		// W1. Exa each nonspacing mark (NSM) in the level run, and change the type of the NSM to the type of the previous character. If the NSM is at the start of the level run, it will get the type of sor.
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	/**
	 * Deter whether a string is empty.
	 ...
	 *   <li>f or F: Fill the path, using the nonzero winding number rule to deter the region to fill.</li>
	 *   <li>f* or F*: Fill the path, using the even-odd rule to deter the region to fill.</li>
	 *   <li>B or FD or DF: Fill and then stroke the path, using the nonzero winding number rule to deter the region to fill.</li>
	 *   <li>B* or F*D or DF*: Fill and then stroke the path, using the even-odd rule to deter the region to fill.</li>
	 *   <li>b or fd or df: Close, fill, and then stroke the path, using the nonzero winding number rule to deter the region to fill.</li>
	 *   <li>b or f*d or df*: Close, fill, and then stroke the path, using the even-odd rule to deter the region to fill.</li>
	 *   <li>CNZ: Clipping mode using the even-odd rule to deter which regions lie inside the clipping path.</li>
	 *   <li>CEO: Clipping mode using the nonzero winding number rule to deter which regions lie inside the clipping path</li>
	 ...
	 *	 <li>D or empty string: Draw (default).</li>
	 *	 <li>F: Fill.</li>
	 *	 <li>F*: Fill using the even-odd rule to deter which regions lie inside the clipping path.</li>
	 *	 <li>DF or FD: Draw and fill.</li>
	 *	 <li>DF* or FD*: Draw and fill using the even-odd rule to deter which regions lie inside the clipping path.</li>
	 *	 <li>CNZ: Clipping mode (using the even-odd rule to deter which regions lie inside the clipping path).</li>
	 *	 <li>CEO: Clipping mode (using the nonzero winding number rule to deter which regions lie inside the clipping path).</li>
	 * </ul>
