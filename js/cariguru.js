...
$(document).ready(function(){
    ...
    $('#adv-search-div-sgr input, #adv-search-div-sgr select').change(function() {
        $("#filterSubmit").trigger("click");
        //$('.cariguru-header > form').submit();
    });

    //this array stores selected guru
    var guru_selected = new Array();
    var blockProcess = false;
    var instructionShown = false;

    //fill from session
    $.post( base_url+'cari_guru/get_guru_pilihan/',{},
        function(data){
            if (data != null) {
                for (var i=0; i<data.id.length; i++) {
                    guru_selected.push({'id' : data.id[i], 'name': data.nama[i], 'gender' : data.gender[i]}); //add to list
                    selectBlock(data.id[i]); // update the block view
                }
            }
                
        },"json");

    //action for pilih button, 
    //on search results and individual popup
    $('.search-result-pilih-sgr, #tentang-pilih-btn').click(function() {
        if (!blockProcess) {
            blockProcess = true;
            var guru_id = $(this).attr('data-guru-id');
            if (!$(this).hasClass('button-cancel')) { //if guru selected
                var guru_name = $(this).attr('data-guru-nama');
                var guru_gender = $(this).attr('data-guru-gender');
                if (guru_selected.length < 7) { //list is not full
                    addSelectedGuru(guru_id, guru_name, guru_gender);
                } else {
                    $('#selected-guru-sgr').addClass('glow')
                                .delay(1200)
                               .queue(function() {
                ...
                removeSelectedGuru(guru_id);
            }    
        }
            
    });

    //action for remove button (selected guru)
    $('#selected-guru-sgr').delegate('.close-btn', 'click', function(e) {
        var guru_id = $(this).parent().prev().attr('data-guru-id');
        removeSelectedGuru(guru_id);
    });

    //drag-drop behaviour
    $('#selected-guru-sgr ul').sortable({
        items: "div.sortable:not(.unclickable, h5, div.small-avatar-wrapper, span, img, i)",
        cursor: 'move',
        start: function (event, ui) {
            $(ui.item).data("last-position", ui.item.index());
        },
        stop: function(ev, ui) {
            var lastPosition = ui.item.data('last-position');
            var newPosition = ui.item.index(); //0: h5, 1: priority, 2: h5, 3-7 other items
            console.log("from: " + lastPosition);
            console.log("to: " + newPosition);
            if (lastPosition == 1 && newPosition > 2) {
                //from priority to other, shift h5 position and switch priority class
                
                $($('div.sortable')[0]).after($('h5#selected-label-sgr')); //switch dom position
                $($('div.sortable')[0]).children('li').addClass('selected-prio-sgr');
                ui.item.children('li').removeClass('selected-prio-sgr');

            } else if (lastPosition > 2 && newPosition == 1) {
                //from other to priority, shift h5 position and switch priority class
                
                $($('div.sortable')[0]).after($('h5#selected-label-sgr')); //switch dom position
                $($('div.sortable')[1]).children('li').removeClass('selected-prio-sgr');
                ui.item.children('li').addClass('selected-prio-sgr');

            } else if (lastPosition > 2 && newPosition == 2) {
                //from other to invalid position for priority, make it the first other
                
                ui.item.before($('h5#selected-label-sgr'));
            } else if (lastPosition == 1 && newPosition == 2) {
                //from priority to first other, move it back to priority
                
                ui.item.after($('h5#selected-label-sgr'));
            }


            //after the drop done, update our guru_selected array and session variable
            var items = $('#selected-guru-sgr').find('li');
            for (var i=0; i<items.length-1; i++) {
                guru_selected[i] = {
                    'id': $(items[i]).attr('data-guru-id'),
                    'name': $(items[i]).children('.selected-name-sgr').text(),
                    'gender': $(items[i]).attr('data-guru-gender')
                }
            }
            $.post( base_url+'cari_guru/update_all_pilih_guru/add/',
                    { 'guru_selected' : guru_selected }
                ).done(function(data){
                    // console.log(data);
                });
        }
    });
    $('#selected-guru-sgr ul').disableSelection();

    //main logic for select guru
    function addSelectedGuru(guru_id, guru_name, guru_gender) {
        if (guru_selected.length == 0) { //first item, show the list
            $('#selected-guru-sgr')
                .css('right','-230px')
                .show()
                .animate({right:'0'},200);
        }
        if (guru_selected.length > 0 && instructionShown == false) { //show instruction
            $('#selected-guru-sgr').popover('destroy');
            $('#selected-guru-sgr').popover({
                content: '<span style="font-family: Ubuntu; color: black;">Anda dapat menukar<br>prioritas guru-guru ini<br>dengan cara <em>drag &amp; drop</em></span>',   
                html: true,
            ...
            $('#selected-guru-sgr').popover('show');
            setTimeout(function () {
                $('#selected-guru-sgr').popover('destroy');
            }, 4000);
        ...
        var found = false;
        for (var i=0; i<guru_selected.length; i++) {
            if (guru_selected[i].id == guru_id) {
                found = true;
                break;
            }
        }
        if (!found) { //guru is not selected yet, select
            $.post( base_url+'cari_guru/update_pilih_guru/add/', //add to session
                    { id : guru_id, nama : guru_name, gender: guru_gender }
                ).done(function(data){
                    guru_selected.push({'id' : guru_id, 'name': guru_name, 'gender' : guru_gender}); //add to list
                    selectBlock(guru_id); // update the block view
                    refreshSelectedView(); //update the list view
                });
    ...
    //main logic for select guru
    function removeSelectedGuru(guru_id) {
        for (var i=0; i<guru_selected.length; i++) {
            if (guru_selected[i].id == guru_id) {
                guru_selected.splice(i,1);
                $.post(base_url+'cari_guru/update_pilih_guru/remove/'+guru_id+'/',{},
                    function(data){
                        // console.log(data);
                    }); //remove from session
                selectBlock(guru_id, false); // update the block view
                refreshSelectedView(); //update the list view
                if (guru_selected.length == 0) { //just deleted last item, hide the list
                    $('#selected-guru-sgr')
                        .stop(true,true)
                        .delay(1200)
                        .animate({right:'-230px'},200);
                }
                if (guru_selected.length > 0 && instructionShown == false) { //show instruction
                    $('#selected-guru-sgr').popover('destroy');
                    $('#selected-guru-sgr').popover({
    ...
    //update the pilih buttons according to selection
    function selectBlock(guru_id, select) {
        select = typeof select !== 'undefined' ? select : true;
        var theButton = $('.search-result-pilih-sgr[data-guru-id="'+guru_id+'"]');
        var theButtonOnPopup = $('#tentang-pilih-btn[data-guru-id="'+guru_id+'"]');

        if (select) {
            $(theButton).parent().next().addClass('blockselected');
            $(theButton).addClass('button-cancel');
            $(theButtonOnPopup).addClass('button-cancel');
            $(theButton).html('<label><input type="checkbox" checked></input><span>Batal</span></label>');
            $(theButtonOnPopup).html('<label><input type="checkbox" checked></input><span>Batal</span></label>');
        } else {
            $(theButton).parent().next().removeClass('blockselected');
            $(theButton).removeClass('button-cancel');
            $(theButtonOnPopup).removeClass('button-cancel');
            $(theButton).html('<label><input type="checkbox"></input><span>Pilih</span></label>');
            $(theButtonOnPopup).html('<label><input type="checkbox"></input><span>Pilih</span></label>');
        }
    }

    //update the list of selected guru
    function refreshSelectedView() {

        //rebuild the list
        $('#selected-guru-sgr ul').empty();
        var num = (guru_selected.length == 0) ? 2 : guru_selected.length + 1;
        for (var i=0; i<num; i++) {
            if (i == 0) {
                $('#selected-guru-sgr ul')
                    .append('<h5 id="selected-priority-label-sgr">Guru yang dipilih: </h5>');
                $('#selected-guru-sgr ul')
                    .append('<div class="sortable unclickable"><li class="selected-prio-sgr"><div class="small-avatar-wrapper" style="display: none;"><img class="small-avatar" src="' + base_url + 'images/default_profile_image.png" /></div><span class="selected-name-sgr"></span></li><span class="btns"><span class="close-btn unclickable"></span></span></div>');
                $('#selected-guru-sgr ul')
                    .append('<h5 id="selected-label-sgr">Guru yang dipertimbangkan: </h5>');
            }
            else if (i < 7) {
                $('#selected-guru-sgr ul')
                    .append('<div class="sortable unclickable"><li><div class="small-avatar-wrapper" style="display: none;"><img class="small-avatar" src="' + base_url + 'images/default_profile_image.png" /></div><span class="selected-name-sgr"></span></li><span class="btns"><span class="close-btn unclickable"></span></span></div>');
            }
        }
        var items = $('#selected-guru-sgr').find('li');
        ...
        //insert the data
        for (var i = 0; i<guru_selected.length; i++) {
            $(items[i]).children('.selected-name-sgr').text(guru_selected[i].name);
            $(items[i]).parent().removeClass('unclickable');
            $($(items[i]).next().children()[0]).append('<i class="glyphicon glyphicon-remove"></i>'); //close button
            $($(items[i]).next().children()[0]).removeClass('unclickable');
            $(items[i]).attr('data-guru-id', guru_selected[i].id);
            $(items[i]).attr('data-guru-gender', guru_selected[i].gender);
            $(items[i]).attr('data-toggle', 'modal');
            $(items[i]).attr('data-target', '#guru-popup-sgr');
            var imgsrc = base_url + "images/pp/" + guru_selected[i].id + ".jpg";
            if (!fileExists(imgsrc)) {
                if (guru_selected[i].gender == 1)
                    imgsrc = base_url + "images/avatarboy.jpg";
                else
                    imgsrc = base_url + "images/avatargirl.jpg";
            }
            $($(items[i]).find('.small-avatar')).attr('src', imgsrc);
            $($(items[i]).children('.small-avatar-wrapper')).show();
        }
    ...
    $('#guru-popup-sgr').on('show.bs.modal', function (e) {
        ...
        $.post(base_url + 'guru/data_guru_json/' + guru_id + '/',{},
                function(data){
                    ...
                    if (data['guru_in_pilihan'] == 1) {
                        $('#tentang-pilih-btn').addClass('button-cancel');
                        $('#tentang-pilih-btn').html('<label><input type="checkbox" checked></input><span>Batal</span></label>');
                    } else {
                        $('#tentang-pilih-btn').removeClass('button-cancel');
                        $('#tentang-pilih-btn').html('<label><input type="checkbox"></input><span>Pilih</span></label>');                        
                    }
    ...
    $('#guru-popup-next-sgr').on('show.bs.modal', function (e) {
        var matpel_id = $("#select-matpel").val();
        var lokasi_id = $("#ddLokasi").val();
        var guru_id = $('.selected-prio-sgr').attr('data-guru-id');
        if(guru_id == null || guru_id == "" || guru_id == 0){
            $('#guru-popup-next-sgr').modal('close');
            return;
        } else {
            var guru_name = $('.selected-prio-sgr .selected-name-sgr').text();
            var guru_gender = $('.selected-prio-sgr').attr('data-guru-gender');
            ...
            $('#matpel-id').val(matpel_id);
            $('#lokasi-id').val(lokasi_id);
            var alt_ids = "";
            var items = $('#selected-guru-sgr li');
            for (var i=0; i<items.length; i++) {
                if (!$(items[i]).hasClass('selected-prio-sgr') && !$(items[i]).parent().hasClass('unclickable')) {
                    alt_ids += $(items[i]).attr('data-guru-id') + ',';
                }
            }
            $('#calon-guru-alt-ids').val(alt_ids);
        }
    });
